...
const roles = (
  await api(
    'GET',
    `/roles?fields=id,name&filter[name][_eq]=${encodeURIComponent(ROLE_NAME)}&limit=1`,
  )
).json.data;
if (!roles?.length) {
  console.log(`role "${ROLE_NAME}" not found — nothing to do`);
  process.exit(0);
}
const roleId = roles[0].id;
const role = (
  await api('GET', `/roles/${roleId}?fields=name,policies.policy.id,policies.policy.admin_access`)
).json.data;
const policy =
  (role.policies || []).map((p) => p.policy).find((p) => p && !p.admin_access) ||
  (role.policies || [])[0]?.policy;
if (!policy) {
  console.log('no policy on Agent role');
  process.exit(1);
}
...
const existing =
  (
    await api(
      'GET',
      `/permissions?fields=collection,action&filter[policy][_eq]=${policy.id}&limit=-1`,
    )
  ).json.data || [];
...
for (const collection of COLLECTIONS) {
  if (has(collection)) {
    console.log(`= read ${collection} already granted`);
    continue;
  }
  ...
  console.log(
    `${r.ok ? '+' : '✗'} read ${collection} (${r.status})${r.ok ? '' : ' ' + JSON.stringify(r.json).slice(0, 200)}`,
  );
}
...
const updatePerm = (
  await api(
    'GET',
    `/permissions?fields=id,fields&filter[policy][_eq]=${policy.id}&filter[collection][_eq]=compensation_requests&filter[action][_eq]=update&limit=1`,
  )
).json.data?.[0];
if (updatePerm) {
  ...
  console.log(
    `${r.ok ? '~' : '✗'} update compensation_requests fields -> [${OPS_EDITABLE_FIELDS.join(',')}] (${r.status})`,
  );
} else {
  const r = await api('POST', '/permissions', {
    policy: policy.id,
  ...
  console.log(
    `${r.ok ? '+' : '✗'} update compensation_requests[${OPS_EDITABLE_FIELDS.join(',')}] (${r.status})${r.ok ? '' : ' ' + JSON.stringify(r.json).slice(0, 200)}`,
  );
}
