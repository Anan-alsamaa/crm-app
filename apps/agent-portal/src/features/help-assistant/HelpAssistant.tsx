...
import { AI_SKIN, Button, cn, Drawer, Pill, Textarea } from '@yiji/ui';
import {
        ...
        <div className="flex h-full min-h-0 flex-col">
          ...
          <div
            ...
            className={cn(
              ...
              /* Bottom-anchored via an AUTO MARGIN on the first message, not
                 `justify-end`.
                 `justify-content: flex-end` on a scroll container overflows
                 past its START edge, and content overflowing the start edge of
                 a flex container cannot be reached by scrolling — the browser
                 reports scrollHeight === clientHeight and there is nothing to
                 scroll to. Measured: three exchanges deep, the first reply sat
                 cut off under the header with no way back to it. That is the
                 "cannot scroll up" this panel was reported for.
                 An auto margin gives the same look — messages at the foot of
                 the panel while the conversation is short — and resolves to
                 zero the moment the content exceeds the box, so scrolling
                 behaves normally. NOT on the opening state: with only the
                 greeting present it would drop Aura's welcome and her starter
                 questions to the floor of a tall empty rectangle. */
              !onlyWelcome && '[&>*:first-child]:mt-auto',
            )}
            aria-live="polite"
          >
            {turns.map((turn, i) =>
