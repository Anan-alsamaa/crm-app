...
const intro = document.getElementById('walk-in-intro') as HTMLElement | null;
...
async function start(phone: string): Promise<void> {
  ...
  try {
    ...
    // The form has done its job — hand the screen to the conversation.
    if (intro) intro.hidden = true;
    YijiChat.init({
      gatewayUrl: SOCKET_URL,
      token: body.token,
      locale: 'en',
      autoOpen: true,
      closeUrl: CLOSE_URL,
...
form?.addEventListener('submit', (e) => {
  ...
  // Matches the gateway's own rule rather than a stricter one: a number typed
  // with spaces or a country code is normal, and rejecting it here would just
  // move the failure somewhere less explainable.
  if (phone.replace(/\D/g, '').length < 7) {
    showError('Please enter the mobile number you order with.');
    return;
  }
  void start(phone);
});
