 ...
 *
 * Reached only through host.ts, which imports this inside a DEV-only branch
 * after finding no walk-in handoff; the published pages build refuses to emit
 * it (vite.pages.config.ts). A walk-in session never gets here.
 *
...
void mintDevToken(identity).then((token) => {
  // autoOpen: the support page opens straight into the chat, so no launcher
  // click stands between the tester and it.
